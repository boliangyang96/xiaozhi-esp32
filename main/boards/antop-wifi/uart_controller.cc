...
UartController::UartController(gpio_num_t tx_pin, gpio_num_t rx_pin, uart_port_t uart_port, int baud_rate)
    ...
    if (Initialize()) {
        ...
        mcp_server.AddTool("self.light.set_led_breath_switch", 
            "Turn on or off the breathing effect of the LED light. When enabled, it provides a medium breathing effect. This tool only controls the on/off state of the LED breathing effect and cannot control the breathing rate or speed.",
            PropertyList({
                Property("state", kPropertyTypeBoolean)
            }),
            [this](const PropertyList& properties) -> ReturnValue {
